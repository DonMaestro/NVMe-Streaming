...
#define PCIE_CFG_PRIM_SEC_BUS   0x00010100
...
static void pcieconf(XDmaPcie *XdmaPciePtr, u16 DeviceId)
{
	u32 dat;
	UINTPTR *baddr;
	UINTPTR *addr;

	baddr = XdmaPciePtr->Config.BaseAddress;
	xil_printf("pcie conf base address 0x%.8X\r\n", baddr);
	dat = *baddr;
	xil_printf("conf 0x%.8X\r\n", dat);


	//baddr = (u32*)((u32)baddr + 0x144);
	xil_printf("pcie conf base address 0x%.8X\r\n", (u32)baddr + 0x144);
	dat = *(u32*)((u32)baddr + 0x144);
	xil_printf("PHY Status/Control Register 0x%.8X\r\n", dat);

	xil_printf("pcie conf base address 0x%.8X\r\n", baddr);
	addr = (u32)baddr + 0x330;
	dat = *addr;
	xil_printf("Nxt Cap, Cap Ver, Loopback VSEC 0x%.8X\r\n", dat);

	for (int i = 1; i < 8; i++) {
		addr = (u32)baddr + 0x144 + i * 4;
		dat = *addr;
...
int PcieInitRootComplex(XDmaPcie *XdmaPciePtr, u16 DeviceId)
{
	...
	//pcieconf(&XdmaPcieInstance, XDMAPCIE_DEVICE_ID);
