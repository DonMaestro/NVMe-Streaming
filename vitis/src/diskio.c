...
)
{
	...
	if(nvmeGetIOSQE()) { return RES_ERROR; }
...
)
{
	...
	while (nvmeGetIOSQE()) {
		nvmeServiceIOCompletions(1);
	}

	if ((BYTE *)0xa0000000 <= buff && (BYTE *)0xb0000000 > buff) {
		nvmeRWStatus = nvmeWriteAsync(buff, (u64) sector, count);
		nvmeSubmitSync();
		comp = 15;
	} else
		nvmeRWStatus = nvmeWrite(buff, (u64) sector, count);
	//nvmeRWStatus = nvmeWriteAsync(buff, (u64) sector, count);
	//nvmeSubmitSync();

	if(nvmeRWStatus != NVME_RW_OK) { return RES_ERROR; }

	while(nvmeGetIOSlip() > comp)
	{
		nvmeServiceIOCompletions(16);
	}
